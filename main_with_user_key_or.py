...
with st.sidebar.expander("About", expanded=True):
    ...
    # It uses the <a href="https://cookbook.openai.com/examples/using_logprobs" target="blank">logprobs</a> feature of the OpenAI API and underlines 
    Eeach token in the response. Brighter red means less certainty.
...
models = ["eipm-gpt-4o","eipm-gpt-4"]
model_name = st.sidebar.radio("Pick a model", options=models, index=0)

#### PARAMETERS AND SETTINGS
endpoint = os.getenv("ENDPOINT_URL", "https://eipm-chat-EUS2.openai.azure.com/")
subscription_key = os.getenv("AZURE_OPENAI_API_KEY", "9f5f8935a54940799c7728933c51f08b")
if model_name == 'eipm-gpt-4':
    deployment = os.getenv("DEPLOYMENT_NAME", "eipm-gpt-4-32k-60Kt")
...
elif model_name == 'eipm-gpt-4o':
    deployment = os.getenv("DEPLOYMENT_NAME", "eipm-gpt-4o")
    client = AzureOpenAI(
        azure_endpoint=endpoint,
        api_key=subscription_key,
...
prompt_obj = Prompt()
prompt_preamble = prompt_obj.get_prompt_preamble(model='gpt-4', inference_type='sdoh_entity')

if cur_text_ := st.chat_input("Input the patient's note"):
    for cur_inference_subtype, _prompt in prompt_obj.yield_inference_subtype_prompt(inference_type='sdoh_entity'):
        #     prompt_dict[cur_inference_subtype] = prompt_preamble + prompt
        if cur_inference_subtype == "Datetime":
            prompt = prompt_preamble + cur_text_ + _prompt
...
if not st.session_state.messages:
    st.write("Now type something in the input down the bottom 👇")
